...
import { getAnunciosByAnuncianteId } from "../../services/anuncios.service";
import { changeEstadoAnuncio } from "../../services/anuncios.service";
/** ---------- UI helpers ---------- */
...
/** ---------- Video helpers (multi plataforma) ---------- */
...
/** ---------- Page ---------- */
export default function AnuncianteAnunciosPage() {
    ...
    const [estado, setEstado] = useState("ALL"); // ALL | BORRADOR | ACTIVO | INACTIVO | EXPIRADO

    async function load() {
        setLoading(true);
        setMsg({ type: "", text: "" });
        try {
            const data = await getAnunciosByAnuncianteId(anuncianteId);
            setItems(data ?? []);
        } catch (e) {
            console.log(e);
            setMsg({ type: "danger", text: "No se pudieron cargar tus anuncios." });
        } finally {
            setLoading(false);
        }
    }
    async function handleToggleEstado(anuncio) {
        ...
        // regla: ACTIVO -> INACTIVO, INACTIVO -> ACTIVO
    ...
    const filtered = useMemo(() => {
        ...
        return (items ?? []).filter((a) => {
            ...
            const matchEstado = estado === "ALL" || (a.estado ?? "").toUpperCase() === estado;
    ...
    return (
        <div>
            <div className="d-flex align-items-center justify-content-between mb-3">
                <div>
                    <h1 className="h4 mb-0">Mis anuncios</h1>
                    <div className="text-muted small">Creá, editá y revisá tus anuncios.</div>
                </div>
                ...
                <div className="card-body">
                    <div className="row g-2 align-items-end">
                        <div className="col-12 col-md-6">
                            <label className="form-label mb-1">Buscar por texto / tipo</label>
                            <div className="input-group">
                                ...
                                <input
                                    ...
                                    placeholder="Ej: promo, descuento  IMAGEN_TEXTO..."
                                />
                                {q && (
            ...
            {!loading && !msg.text && items.length === 0 && (
                <div className="alert alert-secondary">Aún no tenés anuncios. Creá el primero.</div>
            )}
            ...
            {!loading && filtered.length > 0 && (
                <div className="row g-3">
                    {filtered.map((a) => {
                        const menuId = `anuncio-menu-${a.id}`;
                                                ...
                                                <ul className="dropdown-menu dropdown-menu-end" aria-labelledby={menuId}>
                                                    <li>
                                                        <button
                                                            ...
                                                            onClick={() => navigate(`/app/anunciante/anuncios/${a.id}`)}
                                                        >
                                                    ...
                                                    <li>
                                                        <button
                                                            className="dropdown-item"
                                                            onClick={() => navigate(`/app/anunciante/anuncios/${a.id}/editar`)}
                                                        >
                                                    ...
                                                    <li>
                                                        <button
                                                            className="dropdown-item"
                                                            onClick={() => navigate(`/app/anunciante/anuncios/${a.id}/comprar`)}
                                                        >
                                                    ...
                                                    <li>
                                                        <button
                                                            className="dropdown-item"
                                                            onClick={() => navigate(`/app/anunciante/anuncios/${a.id}/pagos`)}
                                                        >
