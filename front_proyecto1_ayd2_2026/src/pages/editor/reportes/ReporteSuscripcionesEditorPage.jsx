...
import { getReporteSuscripcionesEditor } from "../../../services/reportesEditor.service";
...
export default function ReporteSuscripcionesEditorPage() {
  ...
  async function cargarReporte() {
    ...
    try {
      ...
      const data = await getReporteSuscripcionesEditor(editorId, {
  ...
  const totalSuscripciones = useMemo(() => {
    return items.reduce((acc, item) => acc + Number(item?.totalSuscripciones || 0), 0);
  }, [items]);
  ...
  function exportarPdfResumen() {
    ...
    doc.setFontSize(16);
    doc.text("Reporte de suscripciones por revista", 14, 15);
    ...
    autoTable(doc, {
      startY: 28,
      head: [["ID revista", "Título", "Categoría", "Total suscripciones"]],
      body,
      styles: {
        fontSize: 9,
        cellPadding: 2,
    ...
    doc.text(`Total suscripciones: ${totalSuscripciones}`, 14, finalY + 7);

    doc.save("reporte-suscripciones-editor.pdf");
  }
  ...
  function exportarPdfDetalleRevista(item) {
    ...
    doc.setFontSize(16);
    doc.text(`Detalle de suscripciones - Revista ${item?.revista?.id ?? ""}`, 14, 15);
    ...
    const body = (item?.detalles || []).map((d, index) => [
      index + 1,
      d?.username ?? "—",
      formatDate(d?.fechaSuscripcion),
      d?.activa ? "Activa" : "Inactiva",
    ]);

    autoTable(doc, {
      startY: 28,
      head: [["#", "Usuario", "Fecha suscripción", "Estado"]],
      body,
      styles: {
        fontSize: 9,
        cellPadding: 2,
    ...
    const finalY = doc.lastAutoTable.finalY + 10;
    doc.setFontSize(11);
    doc.text(`Total suscripciones: ${item?.totalSuscripciones || 0}`, 14, finalY);

    doc.save(`detalle-suscripciones-revista-${item?.revista?.id || "editor"}.pdf`);
  }
  ...
  return (
    <div className="container-fluid">
      <div className="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        <div>
          <h2 className="fw-bold mb-1">Suscripciones por revista</h2>
          <p className="text-muted mb-0">
            Consulta las suscripciones recibidas por tus revistas en un intervalo de tiempo.
          </p>
        </div>
      ...
      <div className="row g-3 mb-4">
        <div className="col-12 col-md-6">
          <div className="card border-0 shadow-sm h-100">
            <div className="card-body">
              <div className="text-muted small">Revistas con suscripciones</div>
        ...
        <div className="col-12 col-md-6">
          <div className="card border-0 shadow-sm h-100">
            <div className="card-body">
              <div className="text-muted small">Total de suscripciones</div>
      ...
      <div className="card shadow-sm border-0">
        <div className="card-header bg-white border-0 pt-3">
          <h5 className="mb-0 fw-semibold">Suscripciones por revista</h5>
        </div>
              ...
              <tbody>
                ...
                {items.map((item, index) => (
                  <tr key={`${item?.revista?.id || "rev"}-${index}`}>
                    ...
                    <td className="fw-semibold text-primary">{item?.totalSuscripciones ?? 0}</td>
                    <td>
                      <div className="d-flex flex-wrap gap-2">
                        <button
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
              <div className="modal-content border-0 shadow">
                <div className="modal-header">
                  <div>
                    <h5 className="modal-title mb-1">Detalle de suscripciones</h5>
                    <div className="text-muted small">
                  ...
                  <div className="row g-3 mb-3">
                    ...
                    <div className="col-12 col-md-4">
                      <div className="border rounded p-3 h-100">
                        ...
                        <p className="mb-0">
                          <strong>Total suscripciones:</strong> {detalle?.totalSuscripciones ?? 0}
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      <tbody>
                        {detalle?.detalles?.length > 0 ? (
                          detalle.detalles.map((d, index) => (
                            <tr key={`${d?.username || "sus"}-${index}`}>
                              <td>{index + 1}</td>
                              <td className="fw-semibold">{d?.username ?? "—"}</td>
                              <td>{formatDate(d?.fechaSuscripcion)}</td>
                              <td>
                                <span className={`badge ${getEstadoBadgeClass(d?.activa)}`}>
                                  {d?.activa ? "Activa" : "Inactiva"}
                                </span>
                              </td>
                            </tr>
                          ...
                          <tr>
                            <td colSpan="4" className="text-center text-muted py-4">
                              No hay suscripciones para esta revista.
                            </td>
                          </tr>
                        )}
                      </tbody>
